...
if (exports) {
  ...
  console.log(`   ✅ main: ${exports['.']?.import || 'missing'}`);
  console.log(`   ✅ types: ${exports['.']?.types || 'missing'}`);
  console.log(`   ✅ styles: ${exports['./styles']?.import || 'missing'}`);
} else {
...
const typeDeclarations = fs.readFileSync('index.d.ts', 'utf8');
const requiredTypes = [
  'Button',
  'Card',
  'Input',
...
requiredTypes.forEach((type) => {
  const hasType = typeDeclarations.includes(`export declare const ${type}:`);
...
componentFiles.forEach((file) => {
  if (fs.existsSync(file)) {
    ...
    const hasInlineClasses = content.includes('class="') && content.includes('bg-');
